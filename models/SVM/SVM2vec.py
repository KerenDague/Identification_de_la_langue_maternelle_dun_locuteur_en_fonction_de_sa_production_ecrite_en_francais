...
TEXT_COLUMN = 'Texte'
LABEL_COLUMN = 'Langue'
OUTPUT_IMAGE_NAME = 'matrice_confusion_svm_w2v.png'
...
class W2VVectorizer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, size=300, min_count=1, sg=1):
        self.size = size
    ...
    def transform(self, X):
        ...
        for text in X:
            tokens = text.split()
            token_vecs = [self.model.wv[w] for w in tokens if w in self.model.wv]
...
def build_svm_pipeline_w2v():
    return Pipeline([
        ("w2v", W2VVectorizer(size=300)),
...
def plot_confusion_matrix(y_true, y_pred, labels, filename):
    ...
    plt.title('Matrice de Confusion - SVM + Word2Vec', fontsize=16)
...
def main():
    ...
    svm_model = build_svm_pipeline_w2v()
