"""
Classification de texte avec BERT et Random Forest.

Ce script permet de prédire automatiquement la langue d’un texte à partir d’un fichier CSV en utilisant des embeddings issus d’un modèle Transformer (BERT) combiné à un classifieur Random Forest
...
3. Construction d’un pipeline : TransformerVectorizer : convertit les textes en vecteurs BERT ; RandomForestClassifier : classifieur prenant en compte les déséquilibres de classes.
4. Entraînement du pipeline sur l'ensemble d'entrainement
...
TEXT_COLUMN = 'Texte'
LABEL_COLUMN = 'Langue'
OUTPUT_IMAGE_NAME = 'matrice_confusion_bert_rf.png'
...
def plot_confusion_matrix(y_true, y_pred, labels, filename):
    ...
    plt.title('Matrice de Confusion - BERT + RF', fontsize=16)
...
def main():
    parser = argparse.ArgumentParser(description="Classification BERT + Random Forest")
