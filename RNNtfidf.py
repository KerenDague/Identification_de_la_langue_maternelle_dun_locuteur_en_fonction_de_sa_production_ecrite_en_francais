"""
Classification de texte avec TF-IDF et RNN (RNN/GRU/LSTM).

Ce script permet de prédire automatiquement la langue d’un texte à partir d’un fichier CSV en utilisant des vecteurs TF-IDF comme entrée d’un réseau récurrent (RNN, GRU ou LSTM au choix en ligne de commande).
...
def plot_confusion_matrix(y_true, y_pred, labels, filename, rnn_type):
    ...
    labels_int = np.arange(len(labels))  # indices entiers pour confusion_matrix
...
def main():
    ...
    OUTPUT_IMAGE_NAME = f"matrice_confusion_tfidf_{RNN_TYPE}.png"  # nom du fichier dynamique
